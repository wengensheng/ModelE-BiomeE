...
!      integer :: N_DEPTH        !Number of soil layers.  SET IN ENT_INIT
     ...
     &     (/ 0.1,0.27,0.57,1.08,1.97,3.5 /) !GISS GCM
      ...
      integer,parameter :: BAND_VIS = 1 !Band for VIS
...
!#define PFT_MODEL_ENT
...
#else
#ifdef PFT_MODEL_GVSD19
      !************************************************
      !*  ENT PLANT FUNCTIONAL TYPE CONSTANTS         *
      !************************************************
      integer,parameter :: N_PFT = 16
      integer,parameter :: N_OTHER = 3  !snow/ice, bare/spare, water
      integer,parameter :: COVER_SAND = 17
      integer,parameter :: COVER_DIRT = 18
      integer,parameter :: N_SOILCOV = 0 !2-light sand, dark dirt (GISS) 
#else
      !************************************************
      ...
      integer,parameter :: ARCTIC = 6 !NK added. Need to make climate zone PFT attribute.
      ...
      ! Weng, 2-10-2017
      real*8, parameter :: f_abv    = 0.75 ! Fraction of above ground woody bimass
      real*8, parameter :: f_branch = 0.2  ! Fraction of branches in abv woody BM
...
!      integer,parameter :: ptrace = 2  !num. nutrient pools used in CASA resp. routine -PK
     ...
     c             8.5/       ! C:N ratio of passive pool
      ...
      ! Added by Weng for BiomeE vegetation and soil, 07-31-2017
      real*8 :: CN0(NPOOLS)
            data CN0/
     1            30.0,       ! C:N ratio of leaf pool
     2           130.0,       ! C:N ratio of wood pool
     3            55.0,       ! C:N ratio of froot pool
     4            30.0,       ! C:N ratio of surfmet pool
     5            120.0,       ! C:N ratio of surfstr pool
     6            25.0,       ! C:N ratio of soilmet pool
     7            120.0,       ! C:N ratio of soilstr pool
     8           130.0,       ! C:N ratio of cwd pool
     9            12.5,       ! C:N ratio of surfmic pool
     a            12.5,       ! C:N ratio of soilmic pool
     b            80.5,       ! C:N ratio of slow pool
     c            40.5/       ! C:N ratio of passive pool

      integer*8,parameter :: N_CAImax = 9 !max. crown layers, 4/24/2021
      integer*8,parameter :: N_SOM = 5 ! number of pools in  BiomeE new soil BGC, 4/24/2021
      real*8,parameter :: c_LLS = 28.57143 ! yr/ (kg C m-2), 1/LMA0, where LMA0 = 0.035
      real*8,parameter :: fgap = 0.1 ! gaps between trees
      real*8,parameter :: tolerance = 1e-6 ! minimum number of trees in a cohort
      real*8,parameter :: merge_limit = 0.2 ! 0.1 ! 0.2! 0.05 !if abs(c1%dbh-c2%dbh)/c2%dbh then merge c1 and c2
      !real*8,parameter :: max_cohorts = 60
      ! End of Weng
     ...
     &     /)
      ...
      character(len=12), parameter :: albedo_bands(N_BANDS) = (/
     &     "soilalb_VIS0",
     &     "soilalb_NIR1",
     &     "soilalb_NIR2",
     &     "soilalb_NIR3",
     ...
     &     /)
      ...
      character(len=6), parameter :: topo_names(6) = (/
     &     "focean",
     &     "flake ",
     &     "fgrnd ",
     &     "fgice ",
