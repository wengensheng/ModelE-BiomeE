...
#ifdef ENT_ALBEDO
      real*8 :: soilalbedodata(N_BANDS),
     &     albedodir(N_BANDS), albedodif(N_BANDS)
#endif      
      !----Local-------
      integer :: i, n
      ...
      !Assign soil (dead) pools. -- MOVED OUTSIDE
!      call assign_patch_soilcarbon(pp, pft, Tpool_ini)

#ifdef ENT_ALBEDO
       !Assign soil spectral albedo
      !write(0,*) "called assign_patch with ",soilalbedodata(:)
      pp%soil_albedo(:) = soilalbedodata(:)
      pp%albedodir(:) = albedodir(:)
      pp%albedodif(:) = albedodif(:)
      pp%albedo(:)    = albedodif(:)
#endif
...
! ==========================================================================
      subroutine summarize_patch_PPA(pp) ! Weng, 2018/04/18
...
!     Modified by Weng because in PPA, cohort fluxes are individual-based (08-16-2017)
      ...
      do while(ASSOCIATED(cop))
        ...
        pp%nm = nsum !total plant number
        ! Re-calculate LAI fraction of a cohort
        cop%LAI = cop%leafarea * cop%n ! added by Weng, 03-31-2017

        pp%GCANOPY = pp%GCANOPY + cop%GCANOPY *cop%crownarea*cop%n !total, Weng added  "*cop%crownarea*cop%n" because it's canopy's Gs in Weng's model
        pp%IPP = pp%IPP + cop%IPP * cop%n     !total
        pp%GPP = pp%GPP + cop%GPP * cop%n ! Weng    !total
        pp%NPP = pp%NPP + cop%NPP * cop%n  ! Weng   !total
        pp%R_auto = pp%R_auto + cop%R_auto * cop%n  ! Weng      !total
        pp%R_root = pp%R_root + cop%R_root * cop%n  ! Weng      !total
        pp%N_up   = pp%N_up + cop%N_uptake * cop%n  !total
        pp%C_litter = pp%C_litter + cop%C_litter    ! calculated in phenology.f, not used by PPA
!        pp%N_litter = pp%N_litter + cop%N_litter  
!        pp%C_to_Nfix = pp%C_to_Nfix + cop%C_to_Nfix

        ! same for the two cases: PPA or not
         !* BIOMASS POOLS - TOTALS in kgC/individ, Weng
        ! gCindiv_to_kgCm2 = 1d-3 * nc !Convert gC/individ to kg-C/m^2 ! Commented by Weng
        pp%C_fol = pp%C_fol + cop%C_fol * nc          ! gCindiv_to_kgCm2
        pp%N_fol = pp%N_fol + cop%N_fol * nc          ! gCindiv_to_kgCm2
        pp%C_w = pp%C_w + (cop%C_sw  + cop%C_hw) * nc ! gCindiv_to_kgCm2
        pp%N_w = pp%N_w + (cop%N_sw  + cop%N_hw) * nc ! gCindiv_to_kgCm2
        pp%C_lab = pp%C_lab + cop%C_lab * nc          ! gCindiv_to_kgCm2
        pp%N_lab = pp%N_lab + cop%N_lab * nc          ! gCindiv_to_kgCm2
        pp%C_froot = pp%C_froot + (cop%C_froot) * nc  ! gCindiv_to_kgCm2
        pp%N_froot = pp%N_froot + (cop%N_froot) * nc  ! gCindiv_to_kgCm2
        pp%C_root = pp%C_root + 
     &       (cop%C_froot + cop%C_croot) * nc         ! gCindiv_to_kgCm2
        pp%N_root = pp%N_root + 
     &       (cop%N_froot + cop%N_croot) * nc         ! gCindiv_to_kgCm2
        cop%Ntot = cop%N_lab+cop%N_fol+cop%N_sw+cop%N_hw+cop%N_froot
        cop%Ctot = cop%C_lab+cop%C_fol+cop%C_sw+cop%C_hw+cop%C_froot
        pp%Ntot = pp%Ntot + cop%Ntot * nc  !Total Nitrogen of plants
        pp%Ctot  = pp%Ctot  + cop%Ctot  * nc
        ! Total leaf area (patch)
        pp%LAI = pp%LAI + cop%leafarea * nc  !Total
        ...
        pp%h = max(pp%h, cop%h) !pp%h + cop%h * nc  !wtd avg
     ...
     &       + (cop%C_sw + cop%C_hw )* cop%n ! + cop%C_croot
!     &       + (cop%C_sw + cop%C_hw )* cop%n !Weng, 08-12-2017, cop%C_croot is part of C_wood
        else    
     ...
     &        + fracrootCASA(ia)*cop%C_froot * cop%n 
       ...
       ! per tree --> per ground area
       pp%C_growth = pp%C_growth + cop%C_growth * cop%n
       cop => cop%shorter
...
!      pp%n = nsum                          !* Total individuals *!
      ...
      ! Let pp%nm be the total number of plants
      pp%nm = nsum

      end subroutine summarize_patch_PPA
...
!@sum Initialize patch, zeroing variables.
      ...
      !----Local----
            ...
            pp%albedo(:) = undef !0.0d0!## Get GISS albveg ##!
            pp%albedodir(:) = undef !0.0d0
            pp%albedodif(:) = undef !0.0d0
...
!@sum patch_print  Print contents of a patch.
     ...
     &         ,pp%Tpool(m,n,i),pp%Tpool(m,n+1,i),pp%Tpool(m,n+2,i)
      ...
      write(iu,1) prefix,"albedodir" ,pp%  albedodir(:) 
      write(iu,1) prefix,"albedodif" ,pp%  albedodif(:) 
      ...
      write(iu,1) prefix,"C_total	",pp%Ctot
...
!@sum patch_merge_data.  Merge the data in two patches into one patch.
      ...
      pp1%Ctot         =w1*pp1%Ctot         +w2*pp2%Ctot
      ...
      !pp1%albedo, albedodir, albedodif:  Should be recalculated from veg.
